...
{
    unpackBrotliTransforms(RFC_TRANSFORMS.prefixSuffixStorage, RFC_TRANSFORMS.prefixSuffixHeads, RFC_TRANSFORMS.triplets);
}
 ...
 */
export function brotliDecode(bytes: Int8Array, options?: BrotliDecodeOptions): Int8Array {
    const s = new State();
    ...
    const expectedOutputSize = options?.expectedOutputSize;
    if (expectedOutputSize !== undefined) {
        const output = new Int8Array(expectedOutputSize + 1);
        s.output = output;
        s.outputOffset = 0;
        s.outputLength = output.length;
        s.outputUsed = 0;
        decompress(s);
        if (s.outputUsed !== expectedOutputSize) {
            throw new Error("Brotli: decompressed length does not match the expected output size");
        }
        close(s);
        closeInput(s);
        return output.subarray(0, expectedOutputSize);
    }
    let totalOutput = 0;
...
/** Returns the immutable RFC 7932 static dictionary used by the codec. */
export function getBrotliStaticDictionary(): Uint8Array {
