/**
 * PDF417 barcode generation (ISO 15438)
 *
 * 2D stacked barcode. Text / byte / numeric compaction modes,
 * Reed-Solomon error correction over GF(929), 3-cluster bar patterns.
 */

import type { RenderNode, RenderGroup, RenderRect } from '../types/render.js'

// ─── GF(929) Arithmetic ───

const PDF417_GF_EXP = new Uint16Array(929)
const PDF417_GF_LOG = new Uint16Array(929)
;(() => {
  let x = 1
  for (let i = 0; i < 928; i++) {
    PDF417_GF_EXP[i] = x
...
;(() => {
  ...
  for (let i = 0; i < 26; i++) TEXT_UPPER[65 + i] = i   // A-Z
  TEXT_UPPER[32] = 26  // SP

  // Lower: a=0..z=25, SP=26
  for (let i = 0; i < 26; i++) TEXT_LOWER[97 + i] = i   // a-z
  TEXT_LOWER[32] = 26  // SP

  // Mixed: 0=0..9=9, special chars
  for (let i = 0; i < 10; i++) TEXT_MIXED[48 + i] = i   // 0-9
  TEXT_MIXED[38] = 10   // &
  TEXT_MIXED[13] = 11   // CR
  TEXT_MIXED[9] = 12    // HT
  TEXT_MIXED[44] = 13   // ,
  ...
  TEXT_MIXED[61] = 23   // =
  TEXT_MIXED[94] = 24   // ^
  // 25 = latch_punct (not a character)
  TEXT_MIXED[32] = 26   // SP
  // 27 = latch_lower, 28 = latch_upper, 29 = shift_punct (not characters)
  ...
  TEXT_PUNCT[125] = 27  // }
  TEXT_PUNCT[39] = 28   // '
  // 29 = latch_upper (not a character)
})()
...
// Latch/shift codes within sub-modes
// Upper: 27=latch_lower, 28=latch_mixed, 29=shift_punct
// Lower: 27=shift_upper, 28=latch_mixed, 29=shift_punct
// Mixed: 25=latch_punct, 27=latch_lower, 28=latch_upper, 29=shift_punct
// Punctuation: 29=latch_upper

function getSubModeTable(subMode: number): Int8Array {
  if (subMode === SM_UPPER) return TEXT_UPPER
  if (subMode === SM_LOWER) return TEXT_LOWER
  if (subMode === SM_MIXED) return TEXT_MIXED
  return TEXT_PUNCT
}

function encodeTextCompaction(data: string): number[] {
  // Build intermediate value list (sub-mode values + latch/shift codes)
  const values: number[] = []
  let currentMode = SM_UPPER

  for (let i = 0; i < data.length; i++) {
    const ch = data.charCodeAt(i)
    if (ch >= 128) continue // Skip non-ASCII in text mode

    const table = getSubModeTable(currentMode)
    const val = table[ch]!
    ...
    // Character not in current sub-mode - need to switch
    if (TEXT_UPPER[ch]! >= 0) {
      // Switch to Upper
      if (currentMode === SM_LOWER) {
        // Lower → Mixed → Upper
        values.push(28) // latch_mixed
        values.push(28) // latch_upper
        currentMode = SM_UPPER
      } else if (currentMode === SM_MIXED) {
        values.push(28) // latch_upper
        currentMode = SM_UPPER
      } else if (currentMode === SM_PUNCT) {
        values.push(29) // latch_upper
        currentMode = SM_UPPER
      }
      values.push(TEXT_UPPER[ch]!)
    } else if (TEXT_LOWER[ch]! >= 0) {
      // Switch to Lower
      if (currentMode === SM_UPPER) {
        values.push(27) // latch_lower (in Upper, 27 = latch to Lower)
        currentMode = SM_LOWER
      } else if (currentMode === SM_MIXED) {
        values.push(27) // latch_lower
        currentMode = SM_LOWER
      } else if (currentMode === SM_PUNCT) {
        values.push(29) // latch_upper
        values.push(27) // latch_lower
      ...
      values.push(TEXT_LOWER[ch]!)
    } else if (TEXT_MIXED[ch]! >= 0) {
      // Switch to Mixed
      if (currentMode === SM_UPPER) {
        values.push(28) // latch_mixed
        currentMode = SM_MIXED
      } else if (currentMode === SM_LOWER) {
        values.push(28) // latch_mixed
        currentMode = SM_MIXED
      } else if (currentMode === SM_PUNCT) {
        values.push(29) // latch_upper
      ...
      values.push(TEXT_MIXED[ch]!)
    } else if (TEXT_PUNCT[ch]! >= 0) {
      // Use shift to Punctuation (single character)
      if (currentMode === SM_UPPER || currentMode === SM_LOWER || currentMode === SM_MIXED) {
        values.push(29) // shift_punct
      }
      values.push(TEXT_PUNCT[ch]!)
      // Shift does not change currentMode permanently
    }
  }

  // Pad to even count (pairs of values become codewords)
  if (values.length % 2 !== 0) {
    values.push(29) // padding with shift_punct or any valid padding value
  }

  // Convert pairs to codewords: high * 30 + low
...
function encodeByteCompaction(data: Uint8Array): number[] {
  const BI_0 = BigInt(0)
  const BI_256 = BigInt(256)
  const BI_900 = BigInt(900)
  const codewords: number[] = []

  // Process groups of 6 bytes → 5 codewords (base-256 to base-900 conversion)
  let i = 0
  const fullGroups = Math.floor(data.length / 6)

  if (fullGroups > 0) {
    codewords.push(924) // byte compaction mode latch (general)
  }

  for (let g = 0; g < fullGroups; g++) {
    const base = g * 6
    // Compute value = b[0]*256^5 + b[1]*256^4 + ... + b[5]*256^0
    // This exceeds 2^53, so use BigInt for accuracy
    let value = BI_0
    for (let j = 0; j < 6; j++) {
      value = value * BI_256 + BigInt(data[base + j]!)
    }
    // Convert to base 900
    const cws = new Array(5)
    for (let j = 4; j >= 0; j--) {
      cws[j] = Number(value % BI_900)
      value = value / BI_900
    }
    for (let j = 0; j < 5; j++) {
      codewords.push(cws[j]!)
    }
    i = base + 6
  }

  // Remaining bytes (< 6): each byte → one codeword
  if (i < data.length) {
    if (fullGroups === 0) {
      codewords.push(901) // byte compaction mode latch (group of 6 not possible)
    }
    for (; i < data.length; i++) {
...
function encodeNumericCompaction(digits: string): number[] {
  const BI_0 = BigInt(0)
  const BI_900 = BigInt(900)
  const codewords: number[] = []
  codewords.push(902) // numeric compaction mode latch

  // Process groups of up to 44 digits
  // For each group: prepend '1', convert to big integer, convert to base 900
  let i = 0
  while (i < digits.length) {
    const groupLen = Math.min(44, digits.length - i)
    const group = digits.substring(i, i + groupLen)
    i += groupLen

    // Prepend '1' and convert to BigInt
    const numStr = '1' + group
    let n = BigInt(numStr)

    // Convert to base 900
    const cwCount = Math.floor(groupLen / 2.69) + 1 // approximate, we'll compute exact
    const tmp: number[] = []
    while (n > BI_0) {
      tmp.push(Number(n % BI_900))
      n = n / BI_900
    }
    // Reverse to get big-endian order
    for (let j = tmp.length - 1; j >= 0; j--) {
      codewords.push(tmp[j]!)
    }
  }
...
function isTextEncodable(data: string): boolean {
  for (let i = 0; i < data.length; i++) {
    const ch = data.charCodeAt(i)
    if (ch >= 128) return false
    if (TEXT_UPPER[ch]! >= 0) continue
    if (TEXT_LOWER[ch]! >= 0) continue
    if (TEXT_MIXED[ch]! >= 0) continue
    if (TEXT_PUNCT[ch]! >= 0) continue
    return false
  }
  return true
}

function encodePDF417Data(data: string): number[] {
  // Numeric compaction for long digit strings
  if (data.length >= 13 && isAllDigits(data)) {
    return encodeNumericCompaction(data)
  }

  // Text compaction for text-encodable data
  if (isTextEncodable(data)) {
    return encodeTextCompaction(data)
  }

  // Byte compaction for binary data
  const bytes = new Uint8Array(data.length)
  for (let i = 0; i < data.length; i++) {
    bytes[i] = data.charCodeAt(i) & 0xFF
  }
  return encodeByteCompaction(bytes)
}

// ─── Reed-Solomon Error Correction over GF(929) ───

function selectECLevel(dataCount: number): number {
...
function computePDF417EC(dataCodewords: number[], ecLevel: number): number[] {
  const ecCount = 1 << (ecLevel + 1) // 2^(level+1)

  // Build generator polynomial: product of (x - 3^i) for i = 1..ecCount (ISO 15438)
  // Coefficients stored low-order first: gen[0] is constant term
  const gen = new Uint16Array(ecCount + 1)
  gen[0] = 1
  let genLen = 1
  for (let i = 0; i < ecCount; i++) {
    const root = PDF417_GF_EXP[i + 1]!
    // Multiply gen by (x - root) in GF(929)
    // (x - root) = x + (929 - root)
    const negRoot = (929 - root) % 929
    const newLen = genLen + 1
    // Process from high to low to avoid overwriting
    for (let j = genLen; j >= 1; j--) {
      gen[j] = (gen[j - 1]! + gf929Mul(gen[j]!, negRoot)) % 929
    }
    gen[0] = gf929Mul(gen[0]!, negRoot)
    genLen = newLen
  }

  // Polynomial division: divide data polynomial by generator polynomial
  // Remainder gives EC codewords
  const remainder = new Uint16Array(ecCount)
  for (let i = 0; i < dataCodewords.length; i++) {
    const t = (dataCodewords[i]! + remainder[ecCount - 1]!) % 929
    // Shift remainder
    for (let j = ecCount - 1; j >= 1; j--) {
      remainder[j] = (remainder[j - 1]! + 929 - gf929Mul(t, gen[j]!)) % 929
    }
    remainder[0] = (929 - gf929Mul(t, gen[0]!)) % 929
  }

  // Negate and reverse the remainder to get EC codewords
  const ec: number[] = new Array(ecCount)
  for (let i = 0; i < ecCount; i++) {
    ec[i] = (929 - remainder[ecCount - 1 - i]!) % 929
  }
  return ec
}
...
function selectDimensions(totalCodewords: number): { rows: number; cols: number } {
  // totalCodewords = rows * cols
  // Constraints: 3 <= rows <= 90, 1 <= cols <= 30
  // Choose minimal area (rows * cols) that fits, prefer squarish
  let bestRows = 90
  let bestCols = 30
  let bestArea = 90 * 30

  for (let cols = 1; cols <= 30; cols++) {
    const rows = Math.ceil(totalCodewords / cols)
    if (rows < 3 || rows > 90) continue
    const area = rows * cols
    if (area < totalCodewords) continue
    if (area < bestArea) {
      bestArea = area
      bestRows = rows
      bestCols = cols
...
function leftRowIndicator(row: number, rows: number, cols: number, ecLevel: number): number {
  const clusterIdx = row % 3
  const t = Math.floor(row / 3)
  if (clusterIdx === 0) {
    return t * 30 + Math.floor((rows - 1) / 3)
  } else if (clusterIdx === 1) {
    return t * 30 + ecLevel * 3 + ((rows - 1) % 3)
  } else {
    return t * 30 + (cols - 1)
  }
}

function rightRowIndicator(row: number, rows: number, cols: number, ecLevel: number): number {
  const clusterIdx = row % 3
  const t = Math.floor(row / 3)
  if (clusterIdx === 0) {
    return t * 30 + (cols - 1)
...
function bitsToModules(pattern: number, bitCount: number, modules: Uint8Array, offset: number): void {
  for (let i = bitCount - 1; i >= 0; i--) {
    modules[offset + (bitCount - 1 - i)] = (pattern >> i) & 1
  }
}

export function renderPDF417(data: string, options: { x: number; y: number; width: number; height: number }): RenderGroup {
  // 1. Encode data
  const dataCW = encodePDF417Data(data)

  // 2. Select EC level and compute EC codewords count
  const ecLevel = selectECLevel(dataCW.length)
  const ecCount = 1 << (ecLevel + 1)

  // 3. Select dimensions
  // Symbol length descriptor counts: itself + data codewords (before padding)
  // Total capacity = rows * cols = 1 (descriptor) + data + padding + EC
  // We need: rows * cols >= 1 + dataCW.length + ecCount
  const minCodewords = 1 + dataCW.length + ecCount
  const dims = selectDimensions(minCodewords)
  const { rows, cols } = dims

  // 4. Build full codeword stream
  // Symbol length descriptor = total data codewords including descriptor (excluding EC)
  const totalDataSlots = rows * cols - ecCount
  const descriptor = totalDataSlots

  const allData: number[] = new Array(totalDataSlots)
  allData[0] = descriptor
  let idx = 1
  for (let i = 0; i < dataCW.length; i++) {
    allData[idx++] = dataCW[i]!
  }
  // Padding with codeword 900
  while (idx < totalDataSlots) {
    allData[idx++] = 900
  }

  // 5. Compute EC codewords
  const ec = computePDF417EC(allData, ecLevel)

  // 6. Full codeword stream (data + EC)
  const fullCodewords: number[] = new Array(rows * cols)
  for (let i = 0; i < allData.length; i++) {
    fullCodewords[i] = allData[i]!
  }
  for (let i = 0; i < ec.length; i++) {
    fullCodewords[allData.length + i] = ec[i]!
  }

  // 7. Build module matrix
  // Each row: start(17) + left_indicator(17) + cols*17 + right_indicator(17) + stop(18)
  const modulesPerRow = 17 + 17 + cols * 17 + 17 + 18
  const moduleMatrix = new Uint8Array(rows * modulesPerRow)

  for (let r = 0; r < rows; r++) {
    const clusterIdx = r % 3
    const cluster = PDF417_CLUSTERS[clusterIdx]!
    const rowOffset = r * modulesPerRow
    let mIdx = 0

    // Start pattern
    bitsToModules(PDF417_START_PATTERN, 17, moduleMatrix, rowOffset + mIdx)
    mIdx += 17

    // Left row indicator
    const leftInd = leftRowIndicator(r, rows, cols, ecLevel)
    const leftPattern = cluster[leftInd % 929]!
    bitsToModules(leftPattern, 17, moduleMatrix, rowOffset + mIdx)
    mIdx += 17

    // Data codewords
    for (let c = 0; c < cols; c++) {
      const cwIdx = r * cols + c
      const cw = fullCodewords[cwIdx]!
      const pattern = cluster[cw % 929]!
      bitsToModules(pattern, 17, moduleMatrix, rowOffset + mIdx)
      mIdx += 17
    }

    // Right row indicator
    const rightInd = rightRowIndicator(r, rows, cols, ecLevel)
    const rightPattern = cluster[rightInd % 929]!
    bitsToModules(rightPattern, 17, moduleMatrix, rowOffset + mIdx)
    mIdx += 17

    // Stop pattern
    bitsToModules(PDF417_STOP_PATTERN, 18, moduleMatrix, rowOffset + mIdx)
  }

  // 8. Convert to RenderGroup
  ...
  // Render modules - optimize by merging consecutive black modules into single rects
  for (let r = 0; r < rows; r++) {
    const rowOffset = r * modulesPerRow
    ...
    for (let m = 0; m <= modulesPerRow; m++) {
      const isBlack = m < modulesPerRow && moduleMatrix[rowOffset + m] === 1
      if (isBlack) {
