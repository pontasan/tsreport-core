/**
 * SHA-512 and SHA-384 (FIPS 180-4). Pure TypeScript (BigInt-based 64-bit
 * arithmetic) so it runs in both Node and the browser. Used by the PDF AES-256
 * (R6) key-derivation hardened hash (ISO 32000-2 Algorithm 2.B).
 */

const MASK64 = (1n << 64n) - 1n

const K: readonly bigint[] = [
  0x428a2f98d728ae22n, 0x7137449123ef65cdn, 0xb5c0fbcfec4d3b2fn, 0xe9b5dba58189dbbcn,
  0x3956c25bf348b538n, 0x59f111f1b605d019n, 0x923f82a4af194f9bn, 0xab1c5ed5da6d8118n,
  0xd807aa98a3030242n, 0x12835b0145706fben, 0x243185be4ee4b28cn, 0x550c7dc3d5ffb4e2n,
  0x72be5d74f27b896fn, 0x80deb1fe3b1696b1n, 0x9bdc06a725c71235n, 0xc19bf174cf692694n,
  0xe49b69c19ef14ad2n, 0xefbe4786384f25e3n, 0x0fc19dc68b8cd5b5n, 0x240ca1cc77ac9c65n,
  0x2de92c6f592b0275n, 0x4a7484aa6ea6e483n, 0x5cb0a9dcbd41fbd4n, 0x76f988da831153b5n,
  0x983e5152ee66dfabn, 0xa831c66d2db43210n, 0xb00327c898fb213fn, 0xbf597fc7beef0ee4n,
  0xc6e00bf33da88fc2n, 0xd5a79147930aa725n, 0x06ca6351e003826fn, 0x142929670a0e6e70n,
  0x27b70a8546d22ffcn, 0x2e1b21385c26c926n, 0x4d2c6dfc5ac42aedn, 0x53380d139d95b3dfn,
  0x650a73548baf63den, 0x766a0abb3c77b2a8n, 0x81c2c92e47edaee6n, 0x92722c851482353bn,
  0xa2bfe8a14cf10364n, 0xa81a664bbc423001n, 0xc24b8b70d0f89791n, 0xc76c51a30654be30n,
  0xd192e819d6ef5218n, 0xd69906245565a910n, 0xf40e35855771202an, 0x106aa07032bbd1b8n,
  0x19a4c116b8d2d0c8n, 0x1e376c085141ab53n, 0x2748774cdf8eeb99n, 0x34b0bcb5e19b48a8n,
  0x391c0cb3c5c95a63n, 0x4ed8aa4ae3418acbn, 0x5b9cca4f7763e373n, 0x682e6ff3d6b2b8a3n,
  0x748f82ee5defb2fcn, 0x78a5636f43172f60n, 0x84c87814a1f0ab72n, 0x8cc702081a6439ecn,
  0x90befffa23631e28n, 0xa4506cebde82bde9n, 0xbef9a3f7b2c67915n, 0xc67178f2e372532bn,
  0xca273eceea26619cn, 0xd186b8c721c0c207n, 0xeada7dd6cde0eb1en, 0xf57d4f7fee6ed178n,
  0x06f067aa72176fban, 0x0a637dc5a2c898a6n, 0x113f9804bef90daen, 0x1b710b35131c471bn,
  0x28db77f523047d84n, 0x32caab7b40c72493n, 0x3c9ebe0a15c9bebcn, 0x431d67c49c100d4cn,
  0x4cc5d4becb3e42b6n, 0x597f299cfc657e2an, 0x5fcb6fab3ad6faecn, 0x6c44198c4a475817n,
]

const IV_512: readonly bigint[] = [
  0x6a09e667f3bcc908n, 0xbb67ae8584caa73bn, 0x3c6ef372fe94f82bn, 0xa54ff53a5f1d36f1n,
  0x510e527fade682d1n, 0x9b05688c2b3e6c1fn, 0x1f83d9abfb41bd6bn, 0x5be0cd19137e2179n,
]

const IV_384: readonly bigint[] = [
  0xcbbb9d5dc1059ed8n, 0x629a292a367cd507n, 0x9159015a3070dd17n, 0x152fecd8f70e5939n,
  0x67332667ffc00b31n, 0x8eb44a8768581511n, 0xdb0c2e0d64f98fa7n, 0x47b5481dbefa4fa4n,
]

const IV_512_224: readonly bigint[] = [
  0x8c3d37c819544da2n, 0x73e1996689dcd4d6n, 0x1dfab7ae32ff9c82n, 0x679dd514582f9fcfn,
  0x0f6d2b697bd44da8n, 0x77e36f7304c48942n, 0x3f9d85a86a1d36c8n, 0x1112e6ad91d692a1n,
]

const IV_512_256: readonly bigint[] = [
  0x22312194fc2bf72cn, 0x9f555fa3c84c64c2n, 0x2393b86b6f53b151n, 0x963877195940eabdn,
  0x96283ee2a88effe3n, 0xbe5e1e2553863992n, 0x2b0199fc2c85b8aan, 0x0eb72ddc81c52ca2n,
]

function rotr(x: bigint, n: bigint): bigint {
  return ((x >> n) | (x << (64n - n))) & MASK64
}

function sha512core(data: Uint8Array, iv: readonly bigint[], outBytes: number): Uint8Array {
  // Padding: 0x80, zeros, then 128-bit big-endian bit length; block size 128.
  const bitLen = BigInt(data.length) * 8n
  const withOne = data.length + 1
  const padTo = (Math.ceil((withOne + 16) / 128)) * 128
  const msg = new Uint8Array(padTo)
  msg.set(data)
  msg[data.length] = 0x80
  // 128-bit length at the end (we only fill the low 64 bits — lengths fit easily).
  const dv = new DataView(msg.buffer)
  dv.setBigUint64(padTo - 8, bitLen & MASK64)

  const H = iv.slice()
  const W = new Array<bigint>(80)
  for (let off = 0; off < padTo; off += 128) {
    for (let t = 0; t < 16; t++) W[t] = dv.getBigUint64(off + t * 8)
    for (let t = 16; t < 80; t++) {
      const w15 = W[t - 15]!
      const w2 = W[t - 2]!
      const s0 = rotr(w15, 1n) ^ rotr(w15, 8n) ^ (w15 >> 7n)
      const s1 = rotr(w2, 19n) ^ rotr(w2, 61n) ^ (w2 >> 6n)
      W[t] = (W[t - 16]! + s0 + W[t - 7]! + s1) & MASK64
    }
    let a = H[0]!, b = H[1]!, c = H[2]!, d = H[3]!, e = H[4]!, f = H[5]!, g = H[6]!, h = H[7]!
    for (let t = 0; t < 80; t++) {
      const S1 = rotr(e, 14n) ^ rotr(e, 18n) ^ rotr(e, 41n)
      const ch = (e & f) ^ (~e & MASK64 & g)
      const temp1 = (h + S1 + ch + K[t]! + W[t]!) & MASK64
      const S0 = rotr(a, 28n) ^ rotr(a, 34n) ^ rotr(a, 39n)
      const maj = (a & b) ^ (a & c) ^ (b & c)
      const temp2 = (S0 + maj) & MASK64
      h = g; g = f; f = e; e = (d + temp1) & MASK64; d = c; c = b; b = a; a = (temp1 + temp2) & MASK64
    }
    H[0] = (H[0]! + a) & MASK64; H[1] = (H[1]! + b) & MASK64; H[2] = (H[2]! + c) & MASK64; H[3] = (H[3]! + d) & MASK64
    H[4] = (H[4]! + e) & MASK64; H[5] = (H[5]! + f) & MASK64; H[6] = (H[6]! + g) & MASK64; H[7] = (H[7]! + h) & MASK64
  }

  const out = new Uint8Array(64)
  const odv = new DataView(out.buffer)
  for (let i = 0; i < 8; i++) odv.setBigUint64(i * 8, H[i]!)
  return out.subarray(0, outBytes)
}

export function sha512(data: Uint8Array): Uint8Array {
  return sha512core(data, IV_512, 64)
}

export function sha384(data: Uint8Array): Uint8Array {
  return sha512core(data, IV_384, 48)
}

export function sha512_224(data: Uint8Array): Uint8Array {
  return sha512core(data, IV_512_224, 28)
}

export function sha512_256(data: Uint8Array): Uint8Array {
  return sha512core(data, IV_512_256, 32)
}
